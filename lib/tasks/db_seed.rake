...
def db_seed_dogs
  ...
  csv.each do |row|
    breed = row['breed']
    unless AnimalBreed.where(name: breed, animal_specy_id: dog_id).count.positive?
...
def db_seed_cats
  ...
  csv.each do |row|
    breed = row['breed']
    unless AnimalBreed.where(name: breed, animal_specy_id: cat_id).count.positive?
...
def db_seed_birds
  ...
  csv.each do |row|
    breed = row['breed']
    unless AnimalBreed.where(name: breed, animal_specy_id: bird_id).count.positive?
...
def db_seed_rodents
  ...
  csv.each do |row|
    breed = row['breed']
    unless AnimalBreed.where(name: breed, animal_specy_id: rodent_id).count.positive?
...
def db_seed_roles
  ...
  csv.each do |row|
    role = row['role']
    Role.create(name: role) unless Role.where(name: role).count.positive?
  end
end
